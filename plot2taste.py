...
from numpy import min, max, zeros, int, array, arange


def plot2hist(dist1, dist2, BlAreaCut_, fan_range, bl_range, text_label):
    plt.figure(1, figsize=(18, 7))
    plt.subplot(121)
    plt.hist(dist1, bins=100, range=fan_range, alpha=0.75, color='red', log=True)
    ...
    plt.subplot(122)
    plt.hist(dist2, bins=100, range=bl_range, alpha=0.75, color='blue', log=True)
    if BlAreaCut_:
